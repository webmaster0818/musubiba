...
export default function AgencyIndex({ pref, prefName }: { pref: string; prefName: string }) {
  ...
  return (
    <section className="my-8">
      ...
      <AreaInsights db={db} prefName={prefName} />

      {Object.entries(byArea).map(([areaKey, list]) => (
        <div key={areaKey} className="mb-10">
          <h2 className="text-xl font-bold text-[#2C2C2C] mb-4 border-l-4 border-[#A08447] pl-3">{AREA_LABELS[areaKey] || areaKey}エリア({list.length}件)</h2>
              ...
              return (
                <div key={a.slug} className="bg-white rounded-xl border border-gray-100 px-5 py-4 text-sm">
                  <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                    <Link href={`/agency/${encodeURIComponent(a.slug)}/`} className="font-bold text-[#A08447] underline text-base">{a.name}</Link>
                  ...
                  <p className="text-xs mt-1.5">
                    <Link href={`/agency/${encodeURIComponent(a.slug)}/`} className="text-[#A08447] underline">詳細データを見る</Link>
                    {a.mapsUri && (
                      <>
                        <span className="mx-2 text-gray-300">|</span>
                        <a href={a.mapsUri} target="_blank" rel="nofollow noopener noreferrer" className="text-[#A08447] underline">Googleマップ</a>
                      </>
                    )}
                    {brand && (
                      <>
                        <span className="mx-2 text-gray-300">|</span>
                        <Link href={brand.href} className="text-[#A08447] underline">{brand.label}</Link>
                      </>
                    )}
                  </p>
                </div>
          ...
          <ul className="text-sm space-y-1.5">
            {minor.map((a, i) => (
              <li key={i} className="flex flex-wrap gap-x-2">
                ...
                {a.mapsUri && <a href={a.mapsUri} target="_blank" rel="nofollow noopener noreferrer" className="text-[#A08447] underline text-xs">Googleマップ</a>}
              </li>
            ))}
          </ul>
        </div>
